...
chore_hours_suggestions_data AS (
    SELECT
...
INSERT INTO chore_hours_suggestions (resident_id, chore_type, start_timestamp, end_timestamp)
SELECT
    resident_id,
    chore_type,
    start_timestamp,
    suggested_end_timestamp
FROM chore_hours_suggestions_data
WHERE NOT EXISTS (
    SELECT 1
    FROM chore_hours_suggestions AS existing
    WHERE existing.resident_id = chore_hours_suggestions_data.resident_id
      AND existing.chore_type = chore_hours_suggestions_data.chore_type
      AND existing.start_timestamp = chore_hours_suggestions_data.start_timestamp
);